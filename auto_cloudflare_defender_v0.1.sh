...
now=$(date +%s) # 读取系统秒数，大概是这样子的：123812312
...
then
    ...
    # Disable Under Attack Mode 关闭UAM
...
then
    ...
    if [ "$challenge" -eq 1 ]; then
        while :
            ...
            if [ $(echo $result | jq -r '.success') == "true" ]; then # 如果规则创建成功则直接保存
            # 保存规则ID
                filterid=$(echo $result | jq -r '.result[].id')
            else
                filterid=$(echo $result | jq -r '.errors[].meta.id') # 获取导致冲突的规则ID
                for i in $filterid
                do
                # 删除冲突规则
                ...
                if [ $(echo $result1 | jq -r '.success') ]; then
                    ...
                    echo "冲突的规则删除失败或无法获取状态 Error 210"
                fi
            fi
            # 退出循环
